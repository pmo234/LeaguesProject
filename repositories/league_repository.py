...
def save(league):
    sql = "INSERT INTO leagues (name, teams_id) VALUES (%s, %s) RETURNING *"
    values = [league.name, league.teams.id]
...
def select_all():
    ...
    for row in results:
        ...
        league = League(row['name'], team, row['id'] )
...
def select(id):
    ...
    if result is not None:
        ...
        league = League(result['name'], team, result['id'] )
    return league
